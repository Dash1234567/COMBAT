...
- **Periodized calendar** — every plan is split into off-season → pre-season →
  fight camp phases mapped onto real dates, so you always know what to train,
  how to recover, and how to eat on any given day.
...
- **Create Plan flow** — a quick, one-question-at-a-time wizard (sport, goal,
  experience, training days, start date, and optional weight + event targets).
...
COMBAT periodizes every plan. From your **start date** to your **event date**,
the calendar is split into three phases, each with its own workout and recovery
focus:
...
- 🥊 **Fight camp** — peak skills and sparring, keep strength to maintenance, and taper into the event.

The phase date ranges are stored in a `plan_phases` calendar table. The plan page
shows a timeline of the phases on real dates, highlights today's phase and
session, and lets you open any phase to see its week plus its training, recovery
and nutrition guidance. (With no event date, the plan is a single off-season block.)
